...
| files | README.md, and under `src/`: core/diffdrive.h, core/diffdrive.cpp, motion/motion_engine.h, motion/motion_engine.cpp, platform/platform_ports.h, core/heading_wrap.h, core/encoder_glitch_armor.h, motion/odometry.h, platform/nezha_port.h, platform/nezha_port.cpp, platform/otos_port.h, platform/otos_port.cpp, comms/serial_transport.h, comms/serial_transport.cpp, comms/radio_transport.h, comms/radio_transport.cpp, comms/protocol.h, comms/protocol.cpp, comms/wire_handler.h, comms/wire_handler.cpp, comms/wire_adapter.h, comms/wire_adapter.cpp, shims.cpp, blocks/sim.ts, blocks/run.ts, blocks/pose.ts, blocks/stop.ts, blocks/world.ts, blocks/motion.ts |
...
| `omegaFloor` | 20.0 | deg/s | MEASURED tovez 2026-09-04, `captures/bench-acceptance-029-20260904d/omega-floor.log` (`omega-floor.json`): `WHEELS_V ±v ∓v 1500` sweep 70→10 mm/s per wheel, camera-truthed — no hard floor exists with `vMin = 0`; achieved rotation is the full command down to 30 mm/s and ~50 % of command at 10 mm/s; lowest command rotating through both halves of the hold 25 mm/s per wheel. The compiled 20 deg/s sits where the achieved fraction starts to fall off; kept. |
| `lag` | 0.0 | s | MEASURED tovez 2026-09-04, `captures/bench-acceptance-029-20260904d/lag-measure.log` (`lag-trials.json`): four `WHEELS_V ±200 ±200 1500` step responses, first-order fit per wheel against the 400 mm/s² command ramp — left 0.115 s (0.095–0.135), right 0.146 s (0.130–0.165), both wheels settling at 199–206 mm/s. The arrival rule prefers a smaller value than the rise-time fit: `lag 0.13` centres cruise-100 pivots (`g1-run.log`) but under-shoots cruise-70 pivots by 5.9 mm/wheel (`pivot-gates-gain2.log`); `lag 0.095` was used for the translation gates. Compiled default unchanged pending the cross-repo `firmware_bake` for tovez (`lag_s`). |
| `stopDistance` | 0.0 | mm | MEASURED tovez 2026-09-04 as 0: ten alternating floor-cruise (70 mm/s) pivots with `lag 0.13`, `captures/bench-acceptance-029-20260904d/pivot-gates-gain2.log` Phase A, all UNDER-shoot (−2.7…−9.0°, mean −5.9 mm/wheel, sd 2.2) — no positive speed-independent coast exists to record on this drivetrain; the lag term alone over-predicts coast at the floor. Compiled default 0 stands. |
