...
# The three chunks emitHelp() splits today's 19-verb table into. Kept
...
def test_help_golden_vector(wg):
    ...
    dispatcher -- this exact 19-verb listing is ticket 004's own
    catalog: the original 12 plus the six motion verbs
    (WHEELS_X/WHEELS_V/MOVE_X/MOVE_V/GO_TO_R/GO_TO_W) inserted between
    TLM and STOP, matching protocol.md S6's own canonical ordering.
    WHEELS does not appear anywhere in this listing -- WHEELS_V is the
    only spelling.
...
def test_build_help_line_real_table_shape_unchanged(wire_lib):
    """The real 19-verb table, run through buildHelpLine() directly
    with the real 240-byte buffer, must reproduce exactly the same
    reply the HELP verb emitted before 2026-08-27 split its output into
    chunks. buildHelpLine() itself is unchanged and still packs every
    name it is handed onto ONE line -- emitHelp() is what now calls it
    once per chunk, so this direct-call shape is deliberately still the
    single long line."""
