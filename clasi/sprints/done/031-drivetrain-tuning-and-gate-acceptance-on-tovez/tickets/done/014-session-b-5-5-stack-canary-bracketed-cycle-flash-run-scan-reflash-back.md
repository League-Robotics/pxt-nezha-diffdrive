...
## Closing note (team-lead, 2026-09-05): DEFERRED, not executed

The hardware half of this ticket was never run. The build variant it
depends on exists and is tested (`tools/make_deploy.py --fault-spin`,
`tests/tools/test_make_deploy_fault_spin.py`, ticket 013). The scan is
instrumentation for a stack-overflow hypothesis, not a release blocker,
and the session's field time went to the drivetrain findings in
`docs/sprint-031-postmortem.md`. Carried forward as
`clasi/issues/stack-canary-scan-deferred-from-sprint-031.md`, which
records the exact procedure. Closed as deferred so the sprint can close;
none of its acceptance criteria are met and none are claimed.
