...
source_hash: 519f6981f146d63c2fed779de2508b635e2fa3461c0bc4f69e7935f5be88a604
---
# Diff: tools-root-DESIGN.md

Documents sprint 011's planned campaign tooling — the `tour_capture.py`
RUN-vocabulary retarget (the one tool sprint 005's own retargeting work
does not cover) and the new `leg_analysis.py` per-leg believed-vs-target
tool — plus the three bench-handoff campaign procedures (OTOS, residual
leg-fault, brick-reset) that turn that tooling into a repeatable session.
Adds a forward-pointing "Sprint 011 update" note under the existing
"Known limitation — the telemetry gap" section rather than rewriting it,
since most of that section goes stale from sprint 005's concurrent work,
not this sprint's own.
