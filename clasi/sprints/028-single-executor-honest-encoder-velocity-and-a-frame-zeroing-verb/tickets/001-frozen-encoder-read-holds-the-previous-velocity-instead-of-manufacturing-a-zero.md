...
- [ ] `NezhaMotorPort::collect()`'s success branch withholds a fresh
...
- [ ] `src/core/diffdrive.{h,cpp}` are byte-identical to their state
...
- [ ] The held tick still increments `i2cFaultCount_`/`i2cf` — the
  ...
  - [ ] This ticket does not change `i2cf`'s behavior on a genuinely
...
- [ ] Host test (`tests/host/motion_engine_shim.cpp`'s existing
