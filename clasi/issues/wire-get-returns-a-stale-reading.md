...
## Progress, 2026-09-05 (turn-calibration-029 branch close)

Fixed while merging that branch's `turn_calibration.py` into master's,
since the merge already touched this function.

- [x] **`wire_get()` cannot return a value older than its own
      request.** The reply window now opens at the SEND (`t0 =
      time.time()` before `link.seqd()`) and the **last** match in it
      wins, not the first.
- [x] **A regression test writes two different values in quick
      succession and asserts the second read-back is the second
      value.** `tests/calibration/test_turn_calibration_gates.py::test_wire_get_second_read_returns_the_second_value`,
      with a `_FakeLink` that models the defect (old replies stay in
      the transcript). VERIFIED to bite: the old look-back-take-first
      implementation returns 0.958 twice against this fake, the new one
      returns 0.958 then 0.962.
- [x] **The banner reads `stop_distance`.** It asks `stop_distance`
      first and only falls back to `pivot_overrun` when the robot does
      not answer, recording which vocabulary the robot speaks in
      `a.overrun_field` so the printed banner and the suggestion both
      name the field actually read. An unanswered field returns the
      caller's default, and the `stop_distance` probe passes no default
      — so "unknown" is `None` and is distinguishable from a real
      reading of 0.0, which was the specific trap called out above.

`wire_get()` also now retries up to 3 times: a fresh WiFi session drops
the odd first reply (MEASURED vevov 2026-09-04,
`reports/turn-cal-029-20260904/` — the post-flash read came back with
firmware defaults, which reads as a bake that did not take). Retrying
does NOT by itself fix staleness — against the old backward window it
would have re-read the same stale line — so it is an addition to the
window fix, not a substitute for it.

**Not done:** the deeper fix the issue suggests as an alternative —
giving `Link` a request/response correlation for `GET` the way
`seqd()` has one for acks. The window fix is correct for a serialised
caller (every current one), but two concurrent `GET`s of the same field
would still be ambiguous. Left open for whoever touches `Link` next.
